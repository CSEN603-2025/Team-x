...
const [showMyInternships, setShowMyInternships] = useState(false);
  ...
  const internships = [
    { company: 'TechNova Inc.', title: 'Frontend Developer', duration: '3 months' },
    { company: 'DataX Labs', title: 'Data Analyst', duration: '6 weeks' },
    { company: 'Designify', title: 'UI/UX Intern', duration: '2 months' },
  ];
    ...
    <div className={styles.pageWrapper}>
      ...
      <div className={styles.profileContainer}>
        ...
        <div className={styles.section}>
          ...
          {editing ? (
            <textarea name="jobInterests" value={profile.jobInterests} onChange={handleChange} />
          ) : (
        ...
        <div className={styles.section}>
          ...
          {editing ? (
            <textarea name="internships" value={profile.internships} onChange={handleChange} />
          ) : (
        ...
        <div className={styles.section}>
          ...
          {editing ? (
            <textarea name="partTimeJobs" value={profile.partTimeJobs} onChange={handleChange} />
          ) : (
        ...
        <div className={styles.section}>
          ...
          {editing ? (
            <textarea name="collegeActivities" value={profile.collegeActivities} onChange={handleChange} />
          ) : (
          ...
          {editing && (
  <div className={styles.section}>
    <label htmlFor="uploadDocs"><strong>Upload Supporting Documents (PDFs only):</strong></label><br />
    <input
      id="uploadDocs"
      type="file"
      accept=".pdf"
    ...
    {uploadedFiles.length > 0 && (
      <ul style={{ marginTop: '10px' }}>
        ...
        <button onClick={() => (editing ? handleSave() : setEditing(true))}>
          ...
          <div className={styles.section}>
  <button onClick={() => setShowMyInternships(prev => !prev)}>
  ...
  {showMyInternships && (
    <ul style={{ marginTop: '10px' }}>
      {appliedInternships.length === 0 ? (
        <li>You haven’t applied to any internships yet.</li>
      ) : (
        ...
        <div className={styles.section}>
          <button onClick={() => setShowMajors(prev => !prev)}>
...
/>
  {/* New filters directly under major & semester */}
  ...
  <select value={paidFilter} onChange={(e) => setPaidFilter(e.target.value)}>
    ...
    <option value="false">Unpaid</option>
        ...
        <div className={styles.section}>
          <button onClick={() => setShowInternships(prev => !prev)}>
            {showInternships ? 'Hide Internships' : 'View Available Internships in SCAD'}
          </button>
         {showInternships && (
  <div>
    <h4>Filtered Internships:</h4>
    <ul>
      {dummyInternships
        .filter(intern =>
        ...
        .map(intern => (
          <li key={intern.id} style={{ marginBottom: '1em', border: '1px solid #ccc', padding: '10px' }}>
            ...
            <button onClick={() => setSelectedInternship(intern)} style={{ marginTop: '5px' }}>
